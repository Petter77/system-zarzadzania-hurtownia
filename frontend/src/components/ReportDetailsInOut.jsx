...
const ReportDetails = ({ userToken, report, usernames }) => {
  ...
  useEffect(() => {
    const fetchReportItems = async () => {
      try {
        const res = await axios.get(`http://localhost:3000/reports/getReportInOut/${report.id}`, {
          headers: {
        ...
        <table className="w-full table-auto border-collapse border border-gray-300 mb-6">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2 border-b text-left text-gray-700">Typ operacji</th>
              <th className="px-4 py-2 border-b text-left text-gray-700">Wykonana przez</th>
              <th className="px-4 py-2 border-b text-left text-gray-700">Numer seryjny</th>
              <th className="px-4 py-2 border-b text-left text-gray-700">Miejsce docelowe</th>
              <th className="px-4 py-2 border-b text-left text-gray-700">Czas wykonania</th>
            </tr>
          </thead>
          <tbody>
            {reportItems.map(({ id, type, username, serial_number, destination, timestamp}) => (
              <tr key={id}>
                ...
                <td className="px-4 py-2 border-b">{typeTranslations[type] || type}</td>
                <td className="px-4 py-2 border-b">{username}</td>
                <td className="px-4 py-2 border-b">{serial_number}</td>
                <td className="px-4 py-2 border-b">{destination}</td>
                <td className="px-4 py-2 border-b">{new Date(timestamp).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
