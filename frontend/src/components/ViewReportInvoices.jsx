...
import ReportDetails from "./ReportDetailsInvoices";
import ExportReport from "./ExportReportInvoices";
...
const ViewReport = ({ userToken }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await axios.get("http://localhost:3000/reports/getReportsInvoices", {
          headers: {
  ...
  return (
    <div className="container mx-auto p-6">
      <h2 className="text-3xl font-semibold mb-6">Dostępne raporty: faktury</h2>
      {!reports ? (
